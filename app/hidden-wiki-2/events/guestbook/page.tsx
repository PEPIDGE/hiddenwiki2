      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 16, flexWrap: "wrap" }}>
        {EVENTS_GUESTS.map((ev) => (
          <button key={ev.eventId} onClick={() => setExpanded(ev.eventId)}
            style={{ padding: "5px 12px", fontSize: 8, fontFamily: "var(--font-mono)", background: expanded === ev.eventId ? `${ACCENT}22` : "#0d0d0d", color: expanded === ev.eventId ? ACCENT : "#666", border: `1px solid ${expanded === ev.eventId ? ACCENT + "50" : "#1e1e1e"}`, cursor: "pointer", letterSpacing: "0.08em" }}>
            {ev.eventId}
          </button>
        ))}
      </div>
      ...
      {EVENTS_GUESTS.filter((ev) => ev.eventId === expanded).map((ev) => (
        <motion.div key={ev.eventId} initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          <div style={{ padding: "14px 18px", background: "#090909", border: `1px solid ${ev.warning ? ACCENT + "20" : "#1a1a1a"}`, marginBottom: 12 }}>
            <div style={{ fontSize: 8, fontFamily: "var(--font-mono)", color: ev.warning ? ACCENT : "#555", letterSpacing: "0.15em", marginBottom: 6 }}>
            ...
            <div style={{ fontSize: 10, color: "#666", fontFamily: "var(--font-mono)", marginBottom: 12 }}>📍 {ev.location}</div>
            ...
            <button onClick={() => handleSave(ev)}
              style={{ padding: "4px 14px", fontSize: 8, fontFamily: "var(--font-mono)", background: savedClues.includes(`guestbook-${ev.eventId}-event`) ? `${ACCENT}18` : "#111", color: savedClues.includes(`guestbook-${ev.eventId}-event`) ? ACCENT : "#777", border: `1px solid ${savedClues.includes(`guestbook-${ev.eventId}-event`) ? ACCENT + "50" : "#222"}`, cursor: "pointer" }}>
            ...
            <thead>
              <tr style={{ borderBottom: "1px solid #222" }}>
                {["ИМЕ", "ОРГАНИЗАЦИЯ", "СТАТУС", "БЕЛЕЖКИ", ""].map((h) => (
                  <th key={h} style={{ padding: "7px 10px", fontSize: 8, fontFamily: "var(--font-mono)", color: "#444", letterSpacing: "0.12em", textAlign: "left" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {ev.guests.map((g) => (
                <tr key={g.name} style={{ borderBottom: "1px solid #131313", background: (g as any).key ? "#0d0000" : "transparent" }}>
                  ...
                  <td style={{ padding: "9px 10px" }}>
                    <span style={{ fontSize: 9, fontFamily: "var(--font-mono)", color: ORG_COLORS[g.org] || "#666" }}>{g.org}</span>
                  </td>
                  <td style={{ padding: "9px 10px", fontSize: 9, color: g.status === "НЕИЗВЕСТЕН" ? "#FF0033" : "#aaa", fontFamily: "var(--font-mono)" }}>{g.status}</td>
                  <td style={{ padding: "9px 10px", fontSize: 9, color: (g as any).key ? ACCENT : "#666", fontFamily: "var(--font-mono)", maxWidth: 220 }}>{g.note}</td>
                  <td style={{ padding: "9px 10px" }}>
                    {(g as any).key && (
                      <button onClick={() => handleSaveGuest(ev, g.name, g.note)}
                        style={{ padding: "2px 8px", fontSize: 7, fontFamily: "var(--font-mono)", background: savedClues.includes(`guestbook-${ev.eventId}-guest-${g.name.replace(/\s/g, "_")}`) ? `${ACCENT}18` : "#0d0d0d", color: savedClues.includes(`guestbook-${ev.eventId}-guest-${g.name.replace(/\s/g, "_")}`) ? ACCENT : "#555", border: "1px solid #222", cursor: "pointer" }}>
